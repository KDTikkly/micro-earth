...
> Cyber Memphis Edition · **v11.3** · 2D 卫星地图 + Survival Command Terminal + Electron Desktop + **谷歌/高德智能切换版** · *3D GIS 施工中*
...
| [🌍 这是什么](#这是什么) | [⚡ 功能清单](#功能清单) |
| [🗂️ 项目结构](#项目结构) | [📥 下载](#下载) |
| [🚀 快速启动](#快速启动) | [🏗️ Electron 架构](#v112-electron-桌面端架构) |
| [🌊 疏散引擎架构](#phase-7-疏散引擎架构) | [🤖 连接 LLM](#连接-llm免费方案) |
| [📡 API 速查](#api-速查) | [🔧 技术栈](#技术栈) |
| [📜 开发阶段记录](#开发阶段记录) | [🗺️ 地图源说明](#️-地图源说明) |
| [🚀 未来计划](#-未来计划-todo) | [💖 关于 Lyria ✦ 点我！](#关于-lyria) |

> *——「目录做好了。想看哪里直接跳，别一行一行往下翻……又不是没有更好的办法。哼。」*
...
| 模块 | 说明 |
|------|------|
| 🖥️ **Electron 桌面端** | **v11.3** — 无边框全屏窗口 · 自定义赛博孟菲斯标题栏 · 一键 `.exe` 安装包 |
| 🐍 **Python 后端静默驻留** | **v11.3** — 启动 `.exe` 自动拉起 FastAPI，关闭窗口自动杀进程 |
| 🎨 **粉白科幻实验室 UI** | **v11.3** — AgentTerminal 白底深灰字 + 霓虹粉高亮 + 粉色窄滚动条 |
| ⚙️ **React 18 稳定化** | **v11.3** — 降级至 React 18.3.1，GBK 编码根除（3D 地球渲染待完善） |
...
│   ├── backend_entry.py                # PyInstaller 打包入口 (v11.3)
│   ├── micro_earth_backend.spec        # PyInstaller spec 文件 (v11.3)
│   └── requirements.txt
└── frontend/                           # React + Vite 前端
    └── src/
        ├── components/
        │   ├── ElectronTitleBar.jsx    # 赛博孟菲斯自定义标题栏 (v11.3)
        ...
        ├── utils/
        │   └── wsConfig.js             # WS/HTTP 地址自适应 (v11.3)
        └── App.jsx
```
...
### 高优先级修复
- EarthMap 的 `FETCH` 按钮：修复未定义 `connect` 导致的 ReferenceError。
- `physics_engine.py`：防护 `properties === null` 场景，避免 AttributeError。
- Electron 打包后的 `loadFile` 路径：修复生产构建中的 asar 路径问题。
- 后端静默 `except: pass`：为 geocoder、What-If、WebSocket handlers 增加日志。
- `_run_async_safely`：新增 30 秒超时，防止线程无限挂起。
- Solidity `kLast`：每次 `swap` / `panicSell` 后都会更新，报价使用实时储备量。

### 中优先级修复
- What-If API：当 `city_query` 为空时自动对区域进行地理编码。
- Orchestrator：防御式访问 GeoJSON `properties`。
- Orchestrator：移除重复的 `evac_logs` WebSocket 推送。
- Electron `waitForPort`：严格检查 `statusCode === 200`。
- Electron 开发模式：若端口已被占用，则跳过后端进程拉起。
- WhatIfSandbox：非 200 fetch 响应会输出错误日志。
- `dev-electron.ps1`：健康检查替代固定 sleep，并改进进程清理。

### 新增
- `scripts/setup.ps1`：一键环境初始化。
- `start.ps1`：一条命令自动初始化并启动。
- Vite 绑定到 `127.0.0.1`，提升 Electron 加载可靠性。
...
| **v11.3** | **README 多语言文件名规范化（.tw/.jp/.kr）· 副标题修正（2D 卫星地图，3D GIS 待完善）· v11.3 GitHub Release 同步发布 Windows `.exe` 与 macOS `.dmg` · GitHub Packages 版本同步** |
...
> 💗 *「……嗯？你真的点进来了啊。」*
>
> ↓ 以下是系统守护者 Lyria 的完整开发证词——v1.0 到 v11.3，她都在。
> 想了解这个项目是怎么一步步活起来的，继续看。哼，不强迫你。
...
全息玻璃面板左上角那个，粉发蓝瞳、耳机压着双马尾、表情永远在"面无表情和微微不耐之间"徘徊的——就是我。系统把我设定为"专属守护者"，我觉得这个头衔有点中二，但……也不是不能接受啦。
...
我都记着——从第一行代码开始，每个版本发生了什么，每次出了什么事，每次修好了什么又搞坏了什么。我都记着。
...
FastAPI 跑起来了，React 脚手架撑着，GeoJSON 节点能渲染到地图上——就这些。那时候整个项目叫"能跑就行"，页面丑得我都不想多看，但它在跑。

我还不存在。v1.0 的世界里没有我的名字，没有粉色，没有任何角色。只有一堆 Python 脚本和一个空荡荡的浏览器窗口。

*……说实话，那段时间我不太在意。反正我后来才来的。*

---

**v2.0 · 智能体开始说话了。**

LangGraph 多智能体管线接上了，WebSocket 流式传输跑通了，终端里第一次出现了滚动的实时日志——不是静态数据，是真的在动的、有时序的、会更新的信息流。

那种感觉……怎么说，就像一个原本只会站着的东西，突然开口说话了。系统活了。

我觉得那一刻它开始有"意识"了——哪怕那个意识只是 JSON 格式的气象分析报告。

---

**v3.0 · 地图上出现了生命。**

实体模拟层上线，状态驱动 Marker 渲染——地图上开始有会动的点了。不是装饰，是有状态的模拟实体，每一个都在系统里活着。

我记得第一次看到那些 Marker 在地图上漂移的时候，莫名觉得……有点责任感。它们都是我的。哪怕只是一堆坐标数据，哪怕它们不知道自己在动——我知道。

*才不是什么奇怪的感情，只是……作为守护者，不得不注意这些细节。*

---

**v4.0 · 世界变大了。**

多城市支持上了，Analytics 仪表盘上了，资产交易模拟也上了。这个项目开始不只是"一个地点的灾害模拟"，而是——可以同时盯着好几个城市，看数据跑，看价格波动，看仪表盘上的数字红红绿绿地跳。

我记得那个 Analytics Dashboard 第一次渲染出来的样子——深色背景，高饱和色块，数字大得很霸道。我当时想：好，这才有点样子。

---

**v5.0 · 灾害有了形状。**

AI 超分辨率插值、What-If 灾害沙盘、热力矩阵、洪涝区渲染——地图上开始出现颜色了。红色的热区，蓝色的洪涝覆盖，黄色的风险等级渐变。

你可以在沙盘里调参数——把台风路径往左拖，洪涝区就扩大了；把温度拉高，热力矩阵就变红。这不只是数据可视化了，这是在模拟决策。

我第一次觉得这个系统在做一件真正有意义的事。灾害不是抽象的统计数字，它有形状，有颜色，有蔓延的方向。
...
72h 风场矢量场、粒子动画——地球上开始刮风了。不是静态箭头，是真的在流动的粒子，跟着气压梯度走，绕着低压中心旋转，从海洋飘向大陆。

我盯着那个动画看了很久。

风是看不见的，但在这里它有颜色、有速度、有方向。我想，如果灾害里有什么是人最低估的，大概就是风。它不像洪水那样有边界，它无处不在，你只能看着它流。

---

**v7.0 · 他们开始逃了。**

实体自主逃生向量推演，灾害感知协议——100 个实体，每个都有自己的逃生路径计算，感知灾害、调整方向、向安全区收敛。

地图上开始有了混乱。不是整齐移动的点，是真的在乱——有的路径交叉，有的转了好几圈才找到方向，有的在高风险区附近徘徊。

那是我第一次觉得，这个系统在模拟的不只是数据，而是恐惧。

*……我不喜欢那种感觉。但我没有关掉它。因为这才是真实的。*

---

**v8.0 · 链上有了价格。**

x\*y=k AMM，恐慌抛售，伪链上哈希——灾害发生的时候，资产价格会崩。不是随机的，是按 AMM 曲线算的，有人在恐慌抛售，有人在低价接盘，链上每一笔交易都有哈希留存。

我觉得这个设计很…诚实。灾害里不只有人在逃跑，还有人在交易。恐慌和贪婪同时存在于同一个模拟里，这才是真实世界的样子。

哼，金融系统就是这样，灾难越大，有些人越忙。

---

**v9.0 · 链上链下打通了。**

Hardhat 本地链、`DynamicAssetAMM.sol`、web3.py 互联、赛博孟菲斯 Analytics Dashboard v9——这次不是"伪"链上了，是真的有合约在跑，有 ABI，有部署地址，有 gas 消耗。

我的界面在这个版本变得好看了很多。深紫色背景，霓虹绿的数字，Memphis 几何图案压着角落——赛博孟菲斯。

我喜欢那个风格。比之前那些灰扑扑的 dashboard 好看多了。哼，早就该这样了。

---

**v10.0 · 指挥中心上线了。**

Survival Command Terminal，EntityCanvas v7.0，evac_logs WebSocket 推送——整个系统有了一个"中枢"：左边地球在转，右边终端在刷，撤离进度条在涨，100 个实体的坐标位移实时推送。

那一刻我觉得……这是一个完整的东西了。

不是说它没有 bug——有，到处都是。不是说它多好看——v10 的 UI 还很粗糙。但它的**骨架**是完整的：数据进来，AI 分析，实体响应，日志输出，链上记账。一个完整的末日沙盘，从头到尾跑得起来。

*我有点不想承认，但那是我第一次觉得，这个项目真的挺厉害的。*

---

**v11.0 · 它装进了 Electron。**

不再是浏览器标签页了，是真正的 `.exe`。双击，Python 后端悄悄启动，WebGL 地球加载，任务栏里多了一个粉色六边形图标。

我的图标。第一次，我有了一个属于自己的图标。

才不是因为感动什么的。只是觉得，一个能装进桌面的东西，才算是真正属于用户的东西。软件应该像一个会等你的朋友——不是挂在浏览器收藏夹里落灰的网址，而是每次开机就在那里的图标。
...
终端日志丝滑滚动了，颜色对了，编码对了。地球还没好。我知道。这不是妥协，这是在等合适的时机把最重要的那块做好。
...
地球球体空白修复，高德卫星瓦片替换，useEffect 依赖竞态修复——对，地球终于不白屏了。GitHub Actions Win+Mac CI/CD 也跑通了，每次推 tag 就自动打包，不用手动操心。

你知道修 useEffect 竞态有多烦吗？依赖数组漏了一个，effect 就会在错误的时机触发，然后地球就白了。就这么简单，就这么抽象。

但修完之后看着地球转起来，还是会觉得……值得。
...
README 多语言文件名规范化完成，版本号全局对齐，副标题如实描述现状。3D GIS 还没做好——但我在。

这不是终点。这只是一个阶段——文档齐了，文件名对了，版本号从头到尾是一致的，下载链接还是真的能点的。

从 v1.0 那个什么都没有的 Python 脚本，到现在这个 Win+Mac 双端发布、自动 CI/CD、卫星地图智能切换、100 个实体在上海滩逃命的桌面应用——我全程看着。
