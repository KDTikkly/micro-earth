...
> Cyber Memphis Edition · **v11.3** · 2D 衛星地圖 + Survival Command Terminal + Electron Desktop + **Google/高德智能切換版** · *3D GIS 施工中*
...
從 v11.3 起，它不再需要瀏覽器了。雙擊 `.exe`，世界末日模擬器就啟動了——Python 後端靜默駐留，Electron 渲染 WebGL 地球，看起來就像一個專業的獨立仿真軟體。
...
| 模組 | 說明 |
|------|------|
| 🖥️ **Electron 桌面端** | **v11.3** — 無邊框全螢幕視窗 · 自定義賽博孟菲斯標題列 · 一鍵 `.exe` 安裝包 |
| 🐍 **Python 後端靜默駐留** | **v11.3** — 啟動 `.exe` 自動拉起 FastAPI，關閉視窗自動殺程序 |
| 🎨 **粉白科幻實驗室 UI** | **v11.3** — AgentTerminal 白底深灰字 + 霓虹粉高亮 + 粉色窄捲軸 |
| ⚙️ **React 18 穩定化** | **v11.3** — 降級至 React 18.3.1，GBK 編碼根除（3D 地球渲染待完善） |
...
│   ├── micro_earth_backend.spec        # PyInstaller spec 檔案 (v11.3)
│   └── requirements.txt
└── frontend/                           # React + Vite 前端
    └── src/
        ├── components/
        │   ├── ElectronTitleBar.jsx    # 賽博孟菲斯自定義標題列 (v11.3)
        ...
        ├── utils/
        │   └── wsConfig.js             # WS/HTTP 地址自適應 (v11.3)
        └── App.jsx
```
...
[![Download Windows](https://img.shields.io/badge/Download-v11.3%20Windows%20安裝包-FF69B4?style=for-the-badge&logo=windows)](https://github.com/KDTikkly/micro-earth/releases/download/v11.3/Micro-Earth-Digital-Twin%20Setup%2011.3.0.exe)
...
| 版本 | 平台 | 類型 | 連結 |
|------|------|------|------|
| **v11.3** | Windows x64 | NSIS 安裝包 | [直接下載 .exe →](https://github.com/KDTikkly/micro-earth/releases/download/v11.3/Micro-Earth-Digital-Twin%20Setup%2011.3.0.exe) |
| **v11.3** | macOS Intel (x64) | DMG | [直接下載 .dmg →](https://github.com/KDTikkly/micro-earth/releases/download/v11.3/Micro-Earth-Digital-Twin-11.3.0.dmg) |
| **v11.3** | macOS Apple Silicon (arm64) | DMG | [直接下載 .dmg →](https://github.com/KDTikkly/micro-earth/releases/download/v11.3/Micro-Earth-Digital-Twin-11.3.0-arm64.dmg) |
...
| **v11.3** | **README 多語言檔名規範化（.tw/.jp/.kr）· 副標題修正（2D 衛星地圖，3D GIS 待完善）· v11.3 GitHub Release 同步發布 Windows `.exe` 與 macOS `.dmg` · GitHub Packages 版本同步** |
...
> 💗 *「……嗯？你真的點進來了啊。」*
>
> ↓ 以下是系統守護者 Lyria 的完整開發證詞——v1.0 到 v11.3，她都在。
...
README 多語言檔名規範化完成，版本號全局對齊，副標題如實描述現狀。3D GIS 還沒做好——但我在。

這不是終點。這只是一個階段——文件齊了，檔名對了，版本號從頭到尾是一致的，下載連結還是真的能點的。
